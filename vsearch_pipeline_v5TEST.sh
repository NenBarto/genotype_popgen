...
function usage {
    echo "Usage: [WORKDIR] [combined-derep-fasta] [flagged-taxDB-fasta] (opt:SIZE_FILTER, def=10) (opt:SINTAX_CUTOFF, def=0.95)"
    exit
}
...
WORKDIR="$1"
SCRIPTDIR="/Users/svs/CESAR-EnviroDNA/scripts/ngs"
...
fi_TAXDB_FASTA="$3"
SIZE_FILTER="${4:-10}"
SINTAX_CUTOFF="${5:-0.95}"

OUTDIR="$WORKDIR/size"$SIZE_FILTER"-sintax"$SINTAX_CUTOFF""
...
cp "$fi_STARTING_FASTA" "$fo_STARTING_FASTA"

tr '-' '_' < "$fo_STARTING_FASTA" | sed 's/_L001/-L001/g' > "$fo_STARTING_FASTA".filtered
...
then
	...
	echo  "Lists do not match, cannot perform join. Exiting."
	exit
fi
