...
index=$annotationDir/"scaffold61811.fa"
...
mkdir -p $trimDir
bwaDir="$resultsDir/"$projectname".mito.bwa"
mkdir -p $bwaDir


#load files
files=`ls $inDir/*R1.fastq.gz | grep F1.F1`
files=( $files )
...
for file in ${files[@]}; do 
  ...
  fileTrim1gz=$trimDir/$sampleName"_R1_val_1.fq.gz"
  fileTrim2gz=$trimDir/$sampleName"_R2_val_2.fq.gz"
  ...
  mapping_line="bwa mem  $index $fileTrim1gz $fileTrim2gz -o $samFile" 
  sam2bam_line="samtools view -S -b -F 2 $samFile -o $bwaDir/$sampleName.bam"
  ...
  qsub -b y -hold_jid map$sampleName -wd $logDir -j y -N sam2bam$sampleName -R y -pe smp 6 -V $sam2bam_line
